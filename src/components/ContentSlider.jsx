...
const ContentSlider = () => {
  ...
  const nextSlide = () => {
    carouselRef.current?.scroll({ left: carouselRef.current?.scrollLeft + carouselRef.current?.clientWidth });
  ...
  const content = [
    ...
    },
    {
      ...
      description: "Earn by copy trading: Profit by mirroring successful traders, leveraging expertise for potential investment gains.",
    },
    {
      id: 4,
  ...
  useEffect(() => {
    let timeout = setTimeout(() => {
      ...
      if (autoPlay.current) {
        console.log("autplay");
        if (Math.ceil(carouselRef?.current?.scrollLeft + carouselRef?.current?.clientWidth) < carouselRef?.current?.scrollWidth) {
          console.log("next");
          nextSlide();
      ...
      item.isVideo ? (
        ...
        <div className="flex-none flex-shrink-0 w-full md:w-1/2 p-2">
          <div key={item.id} className="w-full border-grad p-[3px] rounded-[24px]">
            <div className="w-full h-full text-center rounded-[22px] py-7 px-8 bg-sidebar text-white">
  ...
  const cannotGoNext =
    Math.ceil(carouselRef?.current?.scrollLeft + carouselRef?.current?.clientWidth) === carouselRef?.current?.scrollWidth;
  ...
  return (
    <div className="mx-auto p-4 flex items-center w-full">
      <button
        className={`p-2 bg-transparent text-white rounded ${cannotGoPrev && "opacity-50 cursor-not-allowed"}`}
        onClick={() => {
      ...
      <div ref={carouselRef} className="flex items-center overflow-x-auto scrollbar-hide w-full">
        {renderLargeContent()}
      </div>

      <button
        className={`p-2 bg-transparent text-white rounded ${cannotGoNext && "opacity-50 cursor-not-allowed"}`}
        onClick={() => {
          autoPlay.current = false;
          nextSlide();
        }}
